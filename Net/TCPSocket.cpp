...
bool TCPSocket::ReceivePacket(void *data, size_t size)
{
	...
	if (mIncomingDataRecvHead - mIncomingDataHead > sizeof(uint32_t))
	{
		...
		const uint32_t availableSpaceInStream = size - writeHead;
	...
	if (!read)
	{
		...
		if (mIncomingDataHead >= mIncomingDataRecvHead) {
			...
			int remainingBytes = mIncomingDataRecvHead - mIncomingDataHead;
...
{
	const int sentBytes = Send((const void*)&mOutgoingData[mOutgoingDataSendHead], mOutgoingDataHead - mOutgoingDataSendHead);
	if (sentBytes > 0)
	{
		...
		if (mOutgoingDataSendHead >= mOutgoingDataHead) {
			...
			int remainingBytes = mOutgoingDataHead - mOutgoingDataSendHead;
